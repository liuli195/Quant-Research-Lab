...
def validate_evidence_matrix(path: Path) -> tuple[ScenarioResult, ...]:
    try:
        table = pq.read_table(path)
    except (OSError, pa.ArrowException) as exc:
        raise ValueError("invalid evidence matrix Parquet") from exc
    if not table.schema.remove_metadata().equals(_SCHEMA):
        raise ValueError("evidence matrix schema mismatch")
    rows = table.to_pylist()
    if table.schema.metadata != _metadata(rows):
        raise ValueError("evidence matrix metadata or digest mismatch")
    ...
    for row in rows:
        try:
            ...
            raise ValueError("evidence matrix JSON fields are invalid") from exc
        if not isinstance(metrics, dict) or not isinstance(reasons, list):
            raise ValueError("evidence matrix JSON fields have invalid types")
        results.append(
            ScenarioResult(
    ...
    if [row.scenario_id for row in results] != sorted(
        row.scenario_id for row in results
    ) or len({row.scenario_id for row in results}) != len(results):
        raise ValueError("evidence matrix scenario IDs are not sorted and unique")
