...
| `research.py` | 通过已验证 Research 链路导出结构化结果 | 页面登录、文件落盘 |
...
jq_sync.py sync-build --strategy <strategy_id> --target <ordinal-or-url>
...
- 策略、构建、回测和模拟交易各自维护 manifest；仓库索引只负责从页面列表定位稳定本地键，不复制数据集明细。
...
- 序号连续且无重复；
- 记录数和文件摘要一致；
- 已结束运行同时存在 `run_start`、`run_end`；
- 活动模拟交易允许暂时没有 `run_end`，但保存最后连续序号和远端游标。

普通控制台日志和性能响应按免费接口持续分页：
...
`auth` 使用仓库外专用 Playwright persistent context（持久上下文）打开可见浏览器，用户自行登录。脚本不导出、打印或提交凭证。生产任务只引用该目录路径；重定向登录页立即返回 `auth_required`。
