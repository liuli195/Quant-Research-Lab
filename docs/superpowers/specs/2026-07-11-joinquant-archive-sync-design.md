...
- 回测完整逐行解析单一 JSONL，并校验单一 Token、连续序号、记录数和摘要；
- 模拟交易根据代码历史的每个 `sourceBacktestId` 取回实际源码，保存历史顺序、操作时间、源 ID、代码 SHA256 和代码文件路径映射；
- 模拟交易保存所有不同归因路径的完整源 JSONL，各源分别校验 Token、连续序号、时间顺序、记录数和摘要；
- 多源记录按模拟交易结果日期范围生成查询视图，范围外历史回测行保留为排除证据；
- 查询视图必须且只能有一个 `run_start`；活动运行不得有 `run_end`，已结束运行必须且只能有一个末尾 `run_end`；
