...
- 归因日志是独立核心数据集；回测按单源校验，模拟交易按全部实际代码版本逐源保存和校验并生成本次运行日期视图；任一源缺失、断序、摘要不符、运行边界冲突或已结束运行缺少 `run_end` 必须阻断门禁。
...
所有响应先调用 `write_raw_gzip`。回测归因日志校验单源 Token 和序号；模拟交易逐代码版本保存源 ID 到完整代码文件映射，逐源校验 Token、序号、记录数和 SHA256，再校验日期视图唯一 `run_start` 和按状态要求的唯一 `run_end`。畸形 JSON 只恢复可明确分割的记录，并把错误偏移和恢复数量写入 manifest。
