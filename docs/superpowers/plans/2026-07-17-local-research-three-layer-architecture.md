...
- 最终态只有 `scripts/research/local_quant_research/vectorbt_runtime.py` 可导入 vectorbt 或其内部类型；策略、结果 writer（写入器）、archive（档案）和 Skill（技能）均不得导入 vectorbt。迁移期间新路径仅供测试，旧生产路径在 Task 10 的同一提交中切换并删除。
...
采集命令必须从项目 `.venv` 启动旧公开入口；fixture 保存摘要、阶段时间、中位峰值内存和历史 `package_bytes` 观测，但该旧整包字段不得直接进入 v2 的 5% 比较。Task 9 必须在旧入口删除前从冻结场景重新派生可对齐的核心/扩展 Parquet 数据载荷基线，并把非同构固定开销单独报告；不提交 `.local` 运行目录。
...
测试两个最小 fixture 模块可由同一加载器加载，并拒绝绝对路径、`..`、仓库外 module file、未知 symbol、源码链接/路径逃逸、旧 `command/project_entry` 字段：
...
`load_strategy()` 只临时把已解析的 `strategy_root` 加入 `sys.path`；导入后验证 `module.__file__` 位于 root 内，并从声明 module 定位顶层包目录（单文件 module 则用文件所在目录），只在该源码边界内静态发现、排序全部普通 `.py` 文件，再恢复 `sys.path`。返回值同时保留模块对象、派生的绝对源码路径和 descriptor，供 code identity（代码身份）与档案复制复用；descriptor 不声明第二份源码清单：
...
源包先由 `validate_result_package()` 只读验证 complete（完整）状态；一次预扫描拒绝 symlink（符号链接）、hardlink（硬链接）、目录连接和非普通文件。目标不存在时使用标准 `shutil.copy2` 复制到同级唯一暂存目录，完成后逐文件核对相对路径、长度与 SHA256，再用 `os.replace()` 发布；失败只删除本次暂存目录。本机同一用户仓库属于可信边界，不建立文件描述符、inode（文件节点）或扫描后敌对换树状态机。
...
配置加载先拒绝未知/旧字段，再解析仓库内路径；外部 JSON 只使用嵌套 `strategy.root/module/symbol`，加载后映射为 `RunConfig.strategy_root/strategy_module/strategy_symbol`。运行身份由冻结的配置、行情 manifest、父进程在当前 module 顶层包内静态发现并排序的全部普通 `.py` 文件、固定共享 runtime 文件集和已安装依赖共同生成，不扫描 `research/archives/`，也不再读取手工输入型 `code-identity.json`。

- [ ] **Step 4: 先实现最小共享 vectorbt runtime 与日常一次冷/热执行**

先在 `test_vectorbt_runtime.py` 写最小无订单 `OrderProgram` 的失败测试，再创建共享 `run_vectorbt()`：它必须调用 `Portfolio.from_order_func()`，隐藏原始 Portfolio（组合），并返回 `ExecutionRun`。本任务验收边界是两个最小 Strategy Module（策略模块）能通过共享 runtime 完成一列、两日、无订单运行；稳定优先级、完整成交回调、惰性缓存与后续程序在 Task 8 的独立失败测试中扩展。
...
Expected: PASS；冷/热扩展使用 PyArrow（列式计算库）现成校验与判等，writer 不再存在第二条预加载校验路径。
...
测试断言 `StrategyDescriptor` 字段中不存在 `source_files`；当前 module 顶层包内全部普通 `.py` 文件按相对路径排序后同时进入 identity（身份）和结果包 `code/`，相邻目录与 `research/archives/` 不进入。第二个最小策略只保留公开 `MODULE`、一次相对导入和无扩展结果，证明标准包导入可用。源码边界测试同时禁止 UUID 私有模块名、全局导入锁和手工 `sys.modules` 清理器。
...
父进程先在受限策略根内解析 module，再以其顶层包目录（单文件 module 则用文件所在目录）为源码边界，发现全部普通 `.py` 文件并拒绝链接、路径逃逸和大小/数量超限；排序结果直接传给运行身份冻结和结果包代码复制。每个 `_execute` 都是全新、只加载一个策略的子进程：把冻结策略根放到 `sys.path` 首位，调用标准 `importlib.import_module()` 后读取 symbol；删除 UUID 命名空间、全局导入锁和手工模块缓存生命周期。
...
测试必须覆盖非法链接/目录连接/非普通文件/硬链接在复制前拒绝、源目标逐字节一致、同内容复用、异内容冲突、目标抢占和复制/校验/发布中断清理。删除只验证文件描述符、inode 竞态和扫描后敌对换树内部状态机的测试。
...
Expected: FAIL，现有实现仍包含超出可信工作区威胁边界的描述符和 inode 状态机。

- [ ] **Step 3: 实现标准扫描、复制、复核和原子发布**

晋升开始前用 `lstat/rglob` 扫描源树并拒绝链接、目录连接、非普通文件和硬链接；使用标准 `shutil.copy2` 复制到目标同级暂存目录，再逐文件复核长度和 SHA256。目标不存在时用 `os.replace` 发布；目标已存在时只做同内容复用或异内容冲突。任何失败只清理本次暂存和新建空父目录，不覆盖现有档案。
...
用两个通用 fixture `OrderProgram` 验证 primary（主运行）与 follow-up（后续运行）都调用同一个 `run_vectorbt()`，第二个运行的 ledger 可作为 final ledger。runtime 不解释冻结计划、执行日可交易性或策略原因码；这些海龟私有语义在 Task 9 组装为 follow-up program。runtime 不得调用 `from_orders()`，不得在 Python 循环中推进现金、仓位或净值。
...
### Task 9: 收敛海龟为单一公开 Strategy Module 并迁移即时与延迟执行
...
把 `vectorbt_inputs.py` 的行情准备与 `vectorbt_callbacks.py` 的 Numba 内核按责任迁到 `_kernel.py`；动作/原因码、单位、共同止损、4/6/12 风险缩放和全量再分配保持策略私有。`prepare()` 必须先验证纯配置，再读取 snapshot（快照）和分配矩阵；缺少声明证据抛 `StrategyEvidenceError(code, message)`，算法错误保留为失败。即时程序与 follow-up（后续）延迟程序都只构造共享 `OrderProgram`，账户变化统一交给 Task 8 的 `run_vectorbt()`。
...
- [ ] **Step 7: 在旧生产文件删除前补齐可比体积基线**

Task 9 只复制逻辑到新私有模块，不修改或删除旧 CLI、engine、writer、delayed 等被替代实现；`__init__.py` 只增加公开 MODULE 导出。先从项目 `.venv` 重跑旧公开入口的三个固定场景，用 3 个冷进程和 5 次预热采样；保留原时间和内存基线，并把旧 `package_bytes` 拆为“同逻辑核心/扩展 Parquet 数据载荷”与“其他固定文件”两项。只有前者写入 `parquet_payload_bytes` 并参与后续 5% 门禁，采集环境与文件集合必须写入 fixture。完成该步骤后 Task 10 才可删除旧入口。
...
### Task 10: 单次切换生产配置并完成 run → package → promote E2E
...
实际提交时保留旧文件中完整 `snapshot_requirements`，不得缩减为示例空对象。源码扫描测试断言：共享目录只有 `vectorbt_runtime.py` 可出现 vectorbt import；旧九个生产文件和策略根手工输入型 `joinquant/strategies/strategy-003/research/code-identity.json` 均不存在；生产配置中没有旧六字段。完成结果包内自动生成的 `config/code-identity.json` 必须保留。
...
先更新配置 v2，再删除列出的九个旧策略文件和策略根手工输入型 `code-identity.json`。不得留 re-export（重新导出）兼容模块、feature flag（功能开关）、旧命令分支或双写路径；测试迁移已在 Task 9 完成，因此删除后应无生产/测试导入残留。
...
先断言 Task 9 已在旧生产文件删除前写入可比的 `parquet_payload_bytes` 和独立固定开销字段，拒绝仍只有旧 `package_bytes` 的基线。测试注入子进程指标，断言冷启动取 3 个全新 PID（进程号）的中位数、同一进程 5 次 warm（热）中位数；时间、峰值内存或同逻辑核心/扩展 Parquet 数据载荷体积任一 `actual > baseline * 1.05` 均失败，任一单次超过 180 秒也失败：

```python
@pytest.mark.parametrize("metric", ("seconds", "peak_working_set_bytes", "parquet_payload_bytes"))
def test_release_gate_rejects_more_than_five_percent(metric: str) -> None:
    baseline = {"seconds": 100.0, "peak_working_set_bytes": 1000, "parquet_payload_bytes": 2000}
...
CLI 增加仅发布验证使用的 `performance release --baseline tests/local_quant_research/fixtures/performance-baseline.json`；依次运行 3,432 × 11、3,432 × 17、延迟 1 日三个场景。每个场景先做完整行为摘要零差异，再比较总阶段时间、峰值 working set 和同逻辑核心/扩展 Parquet 数据载荷字节数；代码、配置、证据和报告等固定自包含开销只单独报告。日常 `run` 代码不得调用此 3/5 采样器。
...
Expected: 三个场景行为摘要零差异；每个冷/热单次小于 180 秒；三个场景的中位时间、峰值进程内存和同逻辑核心/扩展 Parquet 数据载荷体积均不超过各自基线 1.05 倍，固定自包含开销单独列示。若机器环境与 baseline environment 不一致，命令必须拒绝比较并报告 `environment_mismatch`，不得覆盖 baseline。
...
验证报告必须记录：三份 capability（能力规格）的逐项覆盖、所有测试命令与结果、三个场景的零差异摘要、3/5 性能样本、5%/180 秒门禁、峰值内存来源、Parquet 数据载荷体积、固定自包含开销、旧路径残留扫描、无法验证项。只在对应证据真实存在后逐项勾选 `tasks.md`，不得按预估任务总数批量勾选。
