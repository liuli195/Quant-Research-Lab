...
系统 MUST 在固定环境比较重构前后的真实规模、扩展资产和延迟场景。Schema、行数、成交、费用、现金、持仓、净值和逻辑摘要 MUST 完全一致；冷启动、预热、峰值内存和同逻辑核心/扩展 Parquet 数据载荷体积的中位数变化不得超过 5% 测量噪声，并继续满足冷、热各 180 秒绝对门禁。代码、配置、证据和报告等固定自包含开销 MUST 单独报告，不得与旧版非自包含整包直接比较。

正确性基线 MUST 直接从公开 `ExecutionLedger` 和 `ResultExtension` 生成，并用历史 v1 fixture 的 SHA256 记录迁移来源；系统不得为继续执行旧基线测试而保留第二套核心结果适配器。性能环境身份 MUST 至少包含 Python 和关键依赖版本、操作系统发行版与版本、CPU 架构与型号、逻辑核心数和物理内存字节数，且测试 MUST 从规范化环境文档重算 SHA256。

#### Scenario: 真实规模性能回归
- **WHEN** 在相同机器和输入上运行 3,432 日 × 11 ETF 主场景，使用三个冷启动新进程和五次预热采样
- **THEN** 结果证据零差异，时间、内存和同逻辑核心/扩展 Parquet 数据载荷体积均不超过基线 5% 噪声带，固定自包含开销另行列示
