...
系统 MUST 把归因日志作为独立核心数据集，默认完整同步并严格校验，同时完整同步失败运行的错误日志。历史回测 MUST 校验单一日志的 Token（标识）和连续序号；模拟交易 MUST 从全部实际代码版本解析不同归因路径，完整保存每个源 JSONL，逐源校验 Token、连续序号、时间顺序、记录数和摘要，再按本次模拟交易日期范围生成多 Token 查询视图。查询视图必须且只能包含一个 `run_start`；活动运行不得包含 `run_end`，已结束运行必须且只能包含一个末尾 `run_end`。任何源文件缺失、历史代码到源码映射不完整、逐源断序、身份冲突或运行边界不符都 MUST 阻断对象完整性门禁。源代码未实现归因写入器时 MUST 以代码摘要和判断证据标记 `missing_at_source`，不得伪造补全。普通控制台日志和性能响应 MUST 默认获取全部免费内容；只有后续内容存在但无法免费取得或无法证明分页结束时才能标记 `capped_free`。
...
#### Scenario: 模拟交易跨多个代码版本和归因路径
- **WHEN** 模拟交易代码历史包含多个 `sourceBacktestId`，且对应代码使用不同归因 Token 或同一源文件还包含历史回测记录
- **THEN** 系统保存全部源文件及其代码版本映射，逐源校验后只把本次模拟交易日期范围内的记录纳入查询视图；范围外记录作为排除证据保留，不得混入或删除

#### Scenario: 存在写入器但归因日志不完整
- **WHEN** 代码存在归因写入器，但同步结果缺少任一代码版本映射或源文件、源内断序、Token 与路径不一致、记录数或摘要不符、查询视图出现多个运行边界，或已结束运行缺少 `run_end`
- **THEN** 系统将归因日志标记为 `failed` 并阻止对象通过完整性门禁，不得降级为普通日志例外

#### Scenario: 活动模拟交易没有 run_end
- **WHEN** 活动模拟交易的全部归因源文件分别连续，查询视图只有一个 `run_start` 且运行尚未结束
