...
系统 MUST（必须）以仓库内唯一的量化分析 Skill（技能）作为对用户和 Agent（代理）的流程入口；Python（编程语言）命令行接口只能由该 Skill 调用，属于内部执行后端。系统 MUST（必须）输出确定性 JSON（结构化数据）与 Markdown（标记文档）报告，包含来源登记摘要、来源类型、模拟快照身份、能力状态、分析配置、脚本版本、证据矩阵和未适用或证据不足项。分析 MUST NOT（不得）启动、提交、同步、恢复或修改本地研究、聚宽回测、聚宽模拟交易及其归档。
