...
- **THEN** 它只对冻结基线和六个挑战逐次调用单场景本地研究 Skill；调用前以 `preparation_id` 绑定分析计划、基准与七份场景配置，调用后显式登记七组 `scenario_id -> run_id`，校验七份来源共享快照、代码和执行后端，再由准备身份与全部来源摘要派生不可变 `analysis_id`，创建 `analysis-scenarios.json`、`preparation.json` 和只引用七个来源清单、摘要、登记表摘要及共享身份的 `source-results.json`；不得扫描历史目录猜测来源或覆盖旧分析。本地 Skill 不读取分析计划、不接收候选数组、不循环七个方案、不生成聚合清单，其余稳健性只从来源事实计算
