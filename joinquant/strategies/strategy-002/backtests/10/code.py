...
JQ_AUTO_AUDIT_TOKEN = 'etf_dynamic_rebalance-s01-local-shortlist-20260526022126-974a89bd'
...
def snapshot_params():
    return {
        ...
        "volatility_window": g.volatility_window,
        "portfolio_vol_window": g.portfolio_vol_window,
        "target_vol": g.target_vol,
        "max_portfolio_vol_scale": g.max_portfolio_vol_scale,
        "annual_factor": g.annual_factor,
        "gold_trend_w": g.gold_trend_w,
        "gold_rs_w": g.gold_rs_w,
        "gold_riskoff_w": g.gold_riskoff_w,
        "ai_momentum_w": g.ai_momentum_w,
        "ai_trend_w": g.ai_trend_w,
        "ai_volpenalty_w": g.ai_volpenalty_w,
        "ai_drawdown_w": g.ai_drawdown_w,
        "nasdaq_momentum_w": g.nasdaq_momentum_w,
        "nasdaq_trend_w": g.nasdaq_trend_w,
        "nasdaq_riskon_w": g.nasdaq_riskon_w,
        "nasdaq_volpenalty_w": g.nasdaq_volpenalty_w,
        "trend_ma_window": g.trend_ma_window,
        "momentum_window_short": g.momentum_window_short,
        "momentum_window_long": g.momentum_window_long,
        "vol_window_short": g.vol_window_short,
        "vol_window_long": g.vol_window_long,
        "drawdown_window": g.drawdown_window,
        "k": g.k,
        "weight_bounds": list(g.weight_bounds),
        "max_weight_change": g.max_weight_change,
        "rebalance_threshold": g.rebalance_threshold,
        "live_days": g.live_days,
        "benchmark": g.benchmark,
        "audit_token": g.audit_token,
        "audit_path": g.audit_path,
    }
...
def set_parameter(context):
    ...
    g.portfolio_vol_window = 40
    g.target_vol = 0.296639288611
    g.max_portfolio_vol_scale = 1.0
    g.annual_factor = 252
    g.gold_trend_w = 0.5
    g.gold_rs_w = 0.3
...
def daily_check(context):
    ...
    if len(close_prices) < 61:
        ...
        audit_event(
            ...
            effective_days=len(close_prices),
            min_days=61,
            asof_date=getattr(context, "previous_date", None),
        )
        return
    ...
    portfolio_vol, vol_scale = compute_portfolio_vol_scale(
        log_returns,
        constrained_weights,
        g.portfolio_vol_window,
        g.target_vol,
        g.annual_factor,
        g.max_portfolio_vol_scale
    )
    final_weights = constrained_weights * vol_scale
    cash_weight = max(0.0, 1.0 - np.sum(final_weights))
    log.info("满仓约束权重: G=%.3f, A=%.3f, N=%.3f" % (
        constrained_weights[0], constrained_weights[1], constrained_weights[2]
    ))
    log.info("组合波动率: %.4f, 目标波动率: %.4f, 缩放系数: %.4f, 现金权重: %.3f" % (
        portfolio_vol, g.target_vol, vol_scale, cash_weight
    ))
    deviation = np.sum(np.abs(final_weights - current_weights))
    has_positions = np.sum(current_weights) > 1e-10
    audit_event(
        "rebalance_signals",
        context,
        pool=g.etf_pool,
        etf_names=g.etf_names,
    ...
    if has_positions:
        ...
        trigger_reason = "above_threshold"
    else:
        log.info("初始建仓：偏离度 %.4f，执行调仓" % deviation)
        trigger_reason = "initial_position"
    audit_event(
        "rebalance_trigger",
        context,
...
def compute_target_weights(volatilities, factor_scores, k):
    ...
    return weights
def compute_portfolio_vol_scale(log_returns, weights, vol_window, target_vol,
                                annual_factor, max_scale):
    log_returns = np.asarray(log_returns, dtype=float)
    weights = np.asarray(weights, dtype=float)
    if (
        log_returns.ndim != 2
        or len(log_returns) < vol_window
        or len(weights) != log_returns.shape[1]
        or not np.all(np.isfinite(log_returns))
        or not np.all(np.isfinite(weights))
        or np.sum(np.abs(weights)) <= 1e-12
        or vol_window < 2
        or target_vol <= 0
        or annual_factor <= 0
        or max_scale <= 0
        ...
        return 0.0, 1.0
    recent_returns = log_returns[-vol_window:]
    cov = np.cov(recent_returns, rowvar=False, ddof=1)
    cov = np.atleast_2d(cov)
    if cov.shape != (len(weights), len(weights)) or not np.all(np.isfinite(cov)):
        return 0.0, 1.0
    portfolio_var = float(weights @ cov @ weights)
    if not np.isfinite(portfolio_var) or portfolio_var <= 1e-20:
        return 0.0, 1.0
    portfolio_vol = np.sqrt(portfolio_var) * np.sqrt(annual_factor)
    if not np.isfinite(portfolio_vol) or portfolio_vol <= 0:
        return 0.0, 1.0
    scale = min(max_scale, target_vol / portfolio_vol)
    scale = min(1.0, max(0.0, scale))
    if not np.isfinite(scale):
        return float(portfolio_vol), 1.0
    return float(portfolio_vol), float(scale)
