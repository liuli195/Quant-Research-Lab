...
JQ_AUTO_AUDIT_TOKEN = 'etf_factor_rotation-s01-deviation-threshold-scan-20260619173905-4a2614e3'
...
def snapshot_params():
    ...
    return {
        ...
        "DeviationThreshold": _runtime_param(
            "DeviationThreshold",
            PARAM_DEFAULTS["DeviationThreshold"],
        ),
...
def validate_params(params):
    ...
    dev_thr = params.get("DeviationThreshold", 0.0)
    if not isinstance(dev_thr, (int, float)) or isinstance(dev_thr, bool) or dev_thr < 0:
        errors.append("DeviationThreshold must be a non-negative number")
...
def initialize(context):
    ...
    elif g.ExecutionTimingMode == "logic-4-deviation-trigger":
        run_weekly(
            prepare_delay_only_rebalance,
            weekday=1,
            time='open',
...
def set_parameter(context):
    ...
    g.RSRS_N = PARAM_DEFAULTS["RSRS_N"]
    g.RSRS_M = PARAM_DEFAULTS["RSRS_M"]
    ...
    g.ExecutionTimingMode = 'logic-4-deviation-trigger'
    g.DeviationThreshold = 0.08
    g.use_real_price = PARAM_DEFAULTS["use_real_price"]
    g.fq_mode = PARAM_DEFAULTS["fq_mode"]
...
def execute_live_like_rebalance(context):
    ...
    queue.pop(0)
def check_deviation_and_execute(context):
    queue = getattr(g, "pending_rebalances", [])
    if not queue:
        return
    pending = queue[0]
    target_weights = pending["final_weights"]
    pool = pending["pool"]
    params = pending["params"]
    threshold = params.get("DeviationThreshold", 0.0)
    account_value = context.portfolio.total_value
    current_weights = []
    for etf in pool:
        pos = context.portfolio.positions[etf]
        value = pos.total_amount * pos.price if pos.total_amount > 0 else 0
        current_weights.append(value / account_value if account_value > 0 else 0)
    deviation = sum(abs(t - c) for t, c in zip(target_weights, current_weights))
    if deviation > threshold:
        audit_event(
            "deviation_trigger_execute",
            context,
            execution_timing_mode="logic-4-deviation-trigger",
            deviation=deviation,
            threshold=threshold,
            final_weights=target_weights,
            current_weights=current_weights,
        )
        execute_rebalance(context, pool, target_weights, params)
        g.pending_rebalances.pop(0)
    else:
        audit_event(
            "deviation_trigger_skip",
            context,
            execution_timing_mode="logic-4-deviation-trigger",
            deviation=deviation,
            threshold=threshold,
