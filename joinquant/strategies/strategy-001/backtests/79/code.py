...
JQ_AUTO_AUDIT_TOKEN = 'etf_factor_rotation-s01-execution-timing-worktree-20260518211452-6b3b9ee8'
...
def set_parameter(context):
    ...
    g.ExecutionTimingMode = 'logic-2-delay-only'
...
def _same_iso_week(left, right):
    if left is None or right is None:
        return False
    left_iso = left.isocalendar()
    right_iso = right.isocalendar()
    return (left_iso[0], left_iso[1]) == (right_iso[0], right_iso[1])
...
def execute_pending_rebalance(context):
    pending = getattr(g, "pending_rebalance", None)
    if not pending:
        return
    trade_date = _context_trade_date(context)
    ...
    g.pending_rebalance = None
def weekly_check_live_like(context):
    trade_date = _context_trade_date(context)
    asof_date = getattr(context, "previous_date", None)
    if not _same_iso_week(trade_date, asof_date):
        audit_event(
            "live_like_skip",
            context,
            ...
            asof_date=asof_date,
            trade_date=trade_date,
            reason="previous_date_not_in_same_iso_week",
        )
        return
