...
JQ_AUTO_AUDIT_TOKEN = 'etf_factor_rotation-s02-logic3-code-variant-20260701002504-55d17183'
...
PARAM_DEFAULTS = {
    ...
    "MaxTotalWeight": 1.0,
    "ExecutionTimingMode": "logic-3-live-like",
...
def _next_trade_date(trade_date):
    ...
    try:
        ...
        log.warning("get next trade day failed: trade_date=%s error=%s", trade_date, exc)
        return None
    future_days = [
        _as_date(day)
        for day in days
        if _as_date(day) is not None and _as_date(day) > trade_date
    ]
...
def prepare_weekend_close_rebalance(context):
    signal_date = _context_trade_date(context)
    next_trade_date = _next_trade_date(signal_date)
    params = snapshot_params()
    if signal_date is None or next_trade_date is None:
        audit_event(
        ...
        return
    if _same_iso_week(signal_date, next_trade_date):
        audit_event(
            "weekend_close_signal_skip",
            context,
            execution_timing_mode=params["ExecutionTimingMode"],
    ...
    plan["trade_date"] = next_trade_date
    g.pending_rebalances.append(plan)
    audit_event(
...
def execute_weekend_close_rebalance(context):
    ...
    pending = queue[0]
    trade_date = _context_trade_date(context)
    target_trade_date = pending.get("trade_date")
    if trade_date is None or target_trade_date is None or trade_date < target_trade_date:
        audit_event(
