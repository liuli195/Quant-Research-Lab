...
"""
============================================================
ç»æ «æéå¶Ðéæ¬µTF æ¾¶æ°¬æ´çæ¯çéã§ç¥é£ã¯ç´ç»¾æâ¬Ñç®»éæ®å¢é?
ç»æ «æç»«è¯²ç·éæ°¬æç»¾è·¨éªéî â¬ä½¸æºéå­çé²æâ¬ä½¸î¿é¥ç²çéã¦â¬ä¾å¤ç¼?
é«åæ¤éå©æ®éæ¬°I ETFé?59819éå¤â¬ä½ºæ¼é¸?00 ETFé?13100éå¤â¬ä¾ç²é²?ETFé?18880é?
éç¨¿ç¸¾é¬æ²åé?
  çå¬ªå¨éã¦î éãæ"é³æç¬é³ææ±"éå±½å§©é²å¿å¸æ´å¿åç¹?æ¶æç"éå²îéâé©æµ å³°åé°å¶ççº­â¬æµ æ²ç¶é?
  RSRS ç»¾æâ¬Ñæ¨å§ï½æ°é·ã¦å°æ´ï¸¾åé¬Ñåµç¼æ°¬æ¹ªæµ éç¸ç¼æ´ç¯æîæ¥é´æ ¦æ°¦éæ¹ç¹éîæ¤éªè«ç²¦éå¶ç²¨é?
  ç¼å«æå¨ã å§©éå¨å¸¶éåç¼éç¬â¬è®³ç²¨æµ£å¶â¬åå¢¿æµ£æ¬ç²¨æµ£å¶ç¹é£æ¬å¹é²æ¯ç´æ¶å¶å¸éæ¿ç¶æ¶â¬éæ §åå©â²ç²¨é?
å¦¯â³æ½¡éåä¼é?
  - çå¬ªå¨éã¦î éå ¢âæ©å¨æ¤éå¤ç´°120 éã¥æ½ç»¾å¤¸äºæ¶å©å¢ éîåé«å¤ç´0/1 ç»ç»æ
  - éã©åºé«å¤å«¨éå±opKéå¤ç´°æ¾¶æ°¬æéç¸å¸éå¶åéæ¿å§éå¿ç´é«å¤å¢  K é?
  - æ¤åº¨æ«éªåç¯éå ¥â¬åå°éã§å·¼éå¤ç´°è½ çå©ç¬éå®å¸çå©ãéå±¾å°éã§å·¼è¤°æç«´é?
  - RSRS æ·î½îéå å½§éå¿ç¬éçç´éæ¬»igh~Low é¥ç²ç¶ å°¾ éå§å¯é?è³ Rèéå²åé¬Ñåéîå [0, 1]
  - é·ã¦å°æ´ï¸½åµç¼æ°¾ç´éîåºæ¶å¶å§éå¤ç´°æµæå¯éå§åæµ£å¶æé§å§â¬ç¡·ç´çå´æ§éè©åé¬Ñå¢¦é¶?
  - ç¼å«æå¨ã å§©éå¨å¸¶éè®¹ç´éîç¼æ¶å¡ºæéå¤ç´°RawWeight ç¼å«æå¨ã å§©éåªç§´é©î½ç£éåçå§£æç¼é?
éç¨¿ç¸¾éîç´¡é?
  FinalWeight_i = RPWeight_i è³ TrendGate_i è³ RSRSMultiplier_i
                è³ CrowdPenalty_i è³ PortfolioVolScale
çå§ç²¨æ£°æ å·¼éæ°­ç¡éã¥ç´é©æ¨»îéã¤ç«´å¨?
============================================================
"""
import numpy as np
import pandas as pd
...
def set_parameter(context):
    ...
    g.etf_names = ['AI ETF', 'ç»¾è«å¯100ETF', 'æ¦å¯å¾ETF']
    g.MA_long = 120
    g.MomShort = 20
    g.MomMid = 60
    g.MomLong = 120
...
def weekly_check(context):
    ...
    _log_step("TrendGate", "çå¬ªå¨éã¦î ", pool, trend_gates, fmt=".0f")
    momentum_scores = compute_momentum_scores(prices, pool, trend_gates, params)
    _log_step("MomentumScore", "éã©åºéåæ", pool, momentum_scores, fmt=".4f")
    selected = select_topk(momentum_scores, trend_gates, params)
    _log_step("Selected", "TopKéã©â¬?, pool, [1.0 if s else 0.0 for s in selected], fmt=".0f")
    rp_weights = compute_rp_weights(prices, pool, selected, params)
    _log_step("RPWeight", "æ¤åº¨æ«éªåç¯éå®å¸", pool, rp_weights, fmt=".4f")
    rsrs_multipliers = compute_rsrs_multipliers(prices, pool, params)
    _log_step("RSRSMultiplier", "RSRSæ·î½îæ¶æ¨»æ", pool, rsrs_multipliers, fmt=".4f")
    crowd_penalties = compute_crowd_penalties(prices, pool, params)
    _log_step("CrowdPenalty", "é·ã¦å°æ´ï¸½åµç¼?, pool, crowd_penalties, fmt=".4f")
    ...
    log.info("[ç¼å«æå¨ã å§©éå©ç¼éç¶¸ PortfolioVolScale=%.4f", portfolio_vol_scale)
    final_weights = raw_weights * portfolio_vol_scale
    _log_step("FinalWeight", "éâ¬ç¼å æ½é²?, pool, final_weights, fmt=".4f")
