...
JQ_AUTO_AUDIT_TOKEN = 'etf_factor_rotation-s05-official-current-baseline-20260709173450-b83d054a'
...
def after_code_changed(context):
    if not getattr(g, "CashAdjustment20260710_700000Done", False):
        try:
            cash_before = context.portfolio.inout_cash
            inout_cash(700000, pindex=0)
            g.CashAdjustment20260710_700000Done = True
            log.info(
                "cash adjustment completed: amount=700000.00, before=%.2f, after=%.2f",
                cash_before,
                context.portfolio.inout_cash,
            )
            audit_event(
                "cash_adjustment",
                context,
                ...
                cash_after=context.portfolio.inout_cash,
            )
        except Exception as exc:
            log.error("cash adjustment handler failed: %s", str(exc))
