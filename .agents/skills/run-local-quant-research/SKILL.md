...
只编排一次通用本地研究调用。共享脚本负责校验配置、行情身份、单场景结果和摘要；Skill（技能）不解释策略字段，不复制项目逻辑，不读取复数分析计划。完整结果包本身 archive-ready（可归档），无需二次重算或改写。

## 执行前提

- 从仓库根目录运行项目 `.venv`（虚拟环境），不回退系统 Python（编程语言），不静默安装依赖。
- 配置必须声明共享 `snapshot_id`（快照标识）、一个场景、仓库内路径和唯一必需输出。
