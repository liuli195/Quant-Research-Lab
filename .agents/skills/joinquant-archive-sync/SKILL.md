...
- 归因日志默认全量同步。模拟交易会从全部代码历史取回真实源代码，保存每个不同日志路径的完整原始文件，再按该模拟交易日期范围生成可查询视图；逐源校验 Token（标识）、连续序号和时间顺序，并校验整次运行的 `run_start` 与终态 `run_end`。代码没有写入器时只能标记 `missing_at_source`，普通日志不能替代归因日志。
